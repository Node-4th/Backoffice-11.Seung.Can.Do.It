...
describe("TeamInfos Serive Unit Test", () => {
  ...
  test("findTeamInfos Method (SUCCESS)", async () => {
    const mockfindTeamInfo =
      mockTeamInfosRepository.findTeamInfos.mockResolvedValue(teamInfo);

    const findTeamInfos = await teamInfosService.findTeamInfos(
      teamInfo.teamInfoId,
    );
  ...
  test("findTeamInfos Method (Missing TeamInfoId)", async () => {
    await expect(teamInfosService.findTeamInfos(null)).rejects.toThrowError(
      "팀 소개페이지를 선택하세요.",
    );
  });
  ...
  test("findTeamInfos Method (No TeamInfo)", async () => {
    ...
    await expect(
      teamInfosService.findTeamInfos(teamInfo.teamInfoId),
    ).rejects.toThrowError("팀 소개페이지가 없습니다.");
  });
  ...
  test("findTeam Method (Missing Team)", async () => {
    await expect(teamInfosService.findTeam(null)).rejects.toThrowError(
      "팀을 선택하세요.",
    );
  });
  ...
  test("findTeam Method (No Team)", async () => {
    ...
    await expect(
      teamInfosService.findTeam(team.teamId, 1),
    ).rejects.toThrowError("팀이 없습니다.");
  });

  test("findTeam Method (Permission Denied)", async () => {
    mockTeamInfosRepository.findTeam.mockResolvedValue(team);
    await expect(
      teamInfosService.findTeam(team.teamId, 5),
    ).rejects.toThrowError("팀 소개페이지에 대한 권한이 없습니다.");
  });

  test("createTeamInfos Method (SUCCESS)", async () => {
    const mockcreateTeamInfo =
      mockTeamInfosRepository.createTeamInfos.mockResolvedValue(teamInfo);
  ...
  test("createTeamInfos Method (Missing required values)", async () => {
    await expect(
      teamInfosService.createTeamInfos(
        "teamId",
        "teamName",
        null,
        "goals",
  ...
  test("editTeamInfos Method (SUCCESS)", async () => {
    const mockeditTeamInfo =
      mockTeamInfosRepository.editTeamInfos.mockResolvedValue(teamInfo);
  ...
  test("editTeamInfos Method (Missing required values)", async () => {
    await expect(
      teamInfosService.editTeamInfos(
        "teamInfoId",
        "teamName",
        null,
        "goals",
  ...
  test("deleteTeamInfos Method (SUCCESS)", async () => {
    const mockdeleteTeamInfo =
      mockTeamInfosRepository.deleteTeamInfos.mockResolvedValue(teamInfo);

    const deleteTeamInfos = await teamInfosService.deleteTeamInfos(
      teamInfo.teamInfoId,
    );
