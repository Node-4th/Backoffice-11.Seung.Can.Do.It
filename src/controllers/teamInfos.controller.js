export class TeamInfosController {
  ...
  createTeamInfos = async (req, res, next) => {
    try {
      ...
      await this.teamInfosService.findTeam(teamId);
      ...
      return res
        .status(201)
        .json({ success: "true", message: "팀 소개페이지를 작성하였습니다." });
    } catch (err) {
      next(err);
    }
  };
  ...
  editTeamInfos = async (req, res, next) => {
    try {
      ...
      const findTeamInfos =
        await this.teamInfosService.findTeamInfos(teamInfoId);
      await this.teamInfosService.findTeam(findTeamInfos.teamId);
  ...
  deleteTeamInfos = async (req, res, next) => {
    try {
      ...
      return res
        .status(204)
        .json({ success: "true", message: "팀 소개페이지를 삭제했습니다." });
    } catch (err) {
      next(err);
    }
  };
